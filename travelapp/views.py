...
def add_hotel(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('home')  # Change 'hotel_list' to the appropriate URL or view name after form submission
    else:
        form = HotelsForm()

    hotels_list = Hotels.objects.all()  # You may want to fetch existing data to display on the template

    return render(request, 'add_hotel.html', {'form': form, 'hotels_list': hotels_list})
...
@user_passes_test(lambda u: u.is_staff, login_url='add_famous')
def add_famous(request):
    if request.method == 'POST':
        form = FamousForm(request.POST, request.FILES)
        if form.is_valid():
            form.save()
            return redirect('home')  # Change 'index' to the appropriate URL or view name after form submission
    else:
        form = FamousForm()

    Famplace = Famous.objects.all()  # You may want to fetch existing data to display on the template

    return render(request, 'add_places.html', {'form': form, 'Famplace': Famplace})
